...
if ( SERVER ) then
    ...
    function PLUGIN:DoPlayerDeath(ply, attacker, damageinfo)
        ...
        if (hook.Run("ShouldSpawnPlayerCorpse", ply, attacker, damageinfo) == false) then
            return
        end
    ...
    function PLUGIN:OnPlayerCorpseCreated(ply, entity)
    	...
    	if ( ply:IsCombine() or ( ply:IsConscript() and ply:GetCharacter():GetClass() == CLASS_CONSCRIPT_R ) ) then
            return false
        end
