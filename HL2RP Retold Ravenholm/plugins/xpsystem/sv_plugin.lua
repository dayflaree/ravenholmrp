...
function PLUGIN.gainXP(ply, bNotify, bNoDonatorBonus)
    local gain = PLUGIN.gain or 5
    if ( ply:IsDonator() ) and not ( bNoDonatorBonus ) then
...
function PLUGIN:OnNPCKilled(npc, ply, ent)
    if ( IsValid(ply) and ply:IsPlayer() ) then
        PLUGIN.gainXP(ply, false, true)
    end
end